...
export function NavbarDefault() {
  ...
  // refs for dropdown hide timeouts
  const hideEventsTimeout = React.useRef(null);
  const hideResourcesTimeout = React.useRef(null);
  ...
  const navList = (
    <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      <li
        className="relative"
        onMouseEnter={() => {
          clearTimeout(hideEventsTimeout.current);
          setIsEventsOpen(true);
        }}
        onMouseLeave={() => {
          hideEventsTimeout.current = setTimeout(() => {
      ...
      >
        ...
        {isEventsOpen && (
          <ul className="absolute left-0 mt-2 bg-white border border-gray-200 shadow-lg rounded-md py-2 z-10">
            <li>
              <Link
                to="/events/upcomings"
      ...
      <li
        className="relative"
        onMouseEnter={() => {
          clearTimeout(hideResourcesTimeout.current);
          setIsResourcesOpen(true);
        }}
        onMouseLeave={() => {
          hideResourcesTimeout.current = setTimeout(() => {
      ...
      >
        ...
        <Link to="/abouts" className="flex items-center">About Us</Link>
      </Typography>
    </ul>
  );
 
  return (
    <Navbar className="w-full px-4 py-2 lg:px-8 lg:py-4">
